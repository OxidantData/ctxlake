...
/// until `shutdown` resolves — then shut down cleanly (releasing any lease this
/// daemon holds; see `ctxlake-sync`'s own `Daemon::shutdown` doc) and remove the
/// pidfile. Split out from [`run_foreground`] so a test can inject both an
