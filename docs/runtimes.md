...
leaves it `false`: nothing ctxlake does on `pre_tool_call` can decline a call, and a hook
crash should never wedge your agent. An unknown event name gets a did-you-mean warning and is skipped, so a typo fails
visibly.
...
- **No `pre_compact` equivalent**, so a *live-captured* Hermes session has no marker for
  the moment its context was discarded. Tool calls, outcomes and digests are unaffected. The
...
- **`pre_llm_call`/`post_llm_call` envelopes carry `role` but no `content`.** No field name
  ...
  session's bronze record therefore has one `Prompt` and one `Assistant` envelope per
  turn, correctly ordered and timestamped, without the text. `ctxlake import` carries the
  message text, because `state.db` stores it; fix live capture by diffing against a real
  capture.
