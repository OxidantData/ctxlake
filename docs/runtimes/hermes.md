...
Hermes' plugin API exposes every event ctxlake needs to build a full envelope, which is
why this page calls capture **spec-complete**. Spec-complete is not the same claim as
implemented: no ctxlake code talks to Hermes today — `adapters/hermes/ctxlake` does not
exist yet, and `ctxlake-hook` itself is still the Wave 1 scaffold
(`crates/ctxlake-hook/src/main.rs` prints "not yet implemented" and exits). Injection
and blocking are additionally **unconfirmed even on paper** — this page says exactly
where each line falls rather than implying a parity nothing here has earned yet.
...
|---|---|---|
| session start callback | `SessionStart` | Mapped in the plugin API; not yet implemented or run. |
| prompt-submitted callback | `Prompt` | Mapped in the plugin API; not yet implemented or run. |
| tool-call callback | `ToolCall` | Mapped in the plugin API; not yet implemented or run: name, input, result, exit code, and duration are all exposed to a plugin callback on paper. |
| response/turn-complete callback | `Assistant` | Mapped in the plugin API; not yet implemented or run. |
| session end callback | `SessionEnd` | Mapped in the plugin API; not yet implemented or run. |
...
|---|---|---|
| Capture | Spec-complete, not implemented | Every event exists in the plugin API on paper; no ctxlake code exercises it yet |
...
**Read that table as it's written, not optimistically.** "Spec-complete" is a real
claim about what Hermes' plugin API exposes on paper — it is not a claim that ctxlake
has run against it, because nothing has (see the status note at the top: no adapter
code exists yet). "Inject and block need confirming" is not hedging for its own sake —
it's the honest position until someone builds the adapter, runs both paths against an
actual Hermes install, and watches them work. Treating an unconfirmed capability as if
it were confirmed is exactly the kind of overclaim AGENTS.md's house rules single out
as a bug, not a style issue.
...
- **Nothing here is implemented yet, capture included** — see the status note at the
  top. Once the adapter exists, expect capture to be the reliable half (the plugin API
  gives it everything it needs, on paper) and treat briefing injection and tool-call
  blocking as "should work on paper, not yet proven" until someone runs both against a
  live install.
...
- [claude-code.md](claude-code.md) — the reference runtime this is measured against
